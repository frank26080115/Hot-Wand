...
Having this flexibility, the first explored option is to simply turn on the iron for a period of time and turn it off for a period of time. If we do this really fast, like 10ms on, and 10ms off, the user won't notice the interruption in heat, and won't hear the coil whine. Once you get into the 1ms range you might start to hear an annoying 1 kHz tone.

The issue with that idea is that the current will probably spike to 5A anyways and trigger the over-current protection that's set at 3A if you've negotiated for 3A over USB-PD.
...
So the firmware was changed (I started writing the firmware like 2 weeks before manufacturing of the PCB), so that the pattern table now always starts with 12 carrier cycles. The code that generates the pattern now inserts more cycles or more blank time (ie. off time) in order to satisfy an average power being requested, all done during run-time.
