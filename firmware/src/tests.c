...
static void test_nvm_read_physical(uintptr_t address,
                                   hotwand_setup_nvm_t *settings);
...
static bool test_nvm_verbose_save(uint16_t sequence,
                                  uint16_t trace_number);
...
void test_run(void)
{
    ...
    // test_rfgen();
    // test_rfgen_burst();
    // test_nvm_simple(); // do nothing until button press, enable UART, write NVM data and validate a full read back with each button press
    // test_nvm_full_page(); // do nothing until button press, enable UART, nearly fill the NVM page with writes, with 2 left over spaces, don't bother outputting too much to UART during this filling, then call the NVM save function 4 times (with UART verbose trace, validating read back data) such that when we are done, there are two entries in the page after a page erase has happened, the UART messages should be very verbose about what address is used and when the erase happened
}
...
void test_bringup_systick(void)
{
    ...
    uint32_t last_report_ms =
        systick_get_ms() - TEST_STATUS_INTERVAL_MS;
...
void test_rfgen_burst(void)
{
    ...
    for (;;) {
        ...
        if (burst_active &&
            (tip_state ||
             ((uint32_t)(now - burst_started_ms) >=
              TEST_RFGEN_BURST_DURATION_MS))) {
            rfgen_stop();
            burst_active = false;
        }
...
void test_nvm_simple(void)
{
    ...
    for (;;) {
        ...
        if (btn_has_short_press(true)) {
            ...
            if (nvm_read(&current) &&
                test_nvm_settings_equal(&current, &next)) {
                ++sequence;
            }
...
void test_nvm_full_page(void)
{
    ...
    for (sequence = 0U; sequence < fill_count; ++sequence) {
        ...
        if (!nvm_save(&expected) ||
            !nvm_read(&read_back) ||
            !test_nvm_settings_equal(&expected, &read_back)) {
    ...
    test_uart_write_number(
        (uint32_t)(test_nvm_slot_count() - test_nvm_used_slot_count()));
    UART_Write("\r\n");

    for (trace = 0U; trace < TEST_NVM_VERBOSE_SAVE_COUNT; ++trace) {
        if (!test_nvm_verbose_save((uint16_t)(fill_count + trace),
                                   (uint16_t)(trace + 1U))) {
    ...
    UART_Write(test_nvm_used_slot_count() == 2U
                   ? " (PASS)\r\n"
                   : " (FAIL, EXPECTED 2)\r\n");
                                    ...
                                    const hotwand_setup_nvm_t *right)
{
    return (left != NULL) && (right != NULL) &&
           (left->startup_power_level == right->startup_power_level) &&
           (left->fan_mode == right->fan_mode) &&
...
static uint16_t test_nvm_slot_count(void)
{
    return (uint16_t)((test_nvm_page_end() - test_nvm_page_start()) /
                      sizeof(hotwand_setup_nvm_t));
}

static uintptr_t test_nvm_slot_address(uint16_t slot)
{
    return test_nvm_page_start() +
           ((uintptr_t)slot * sizeof(hotwand_setup_nvm_t));
}

static bool test_nvm_slot_is_erased(uint16_t slot)
{
    const volatile uint16_t *flash =
        (const volatile uint16_t *)test_nvm_slot_address(slot);
    uint16_t halfword;

    for (halfword = 0U;
         halfword < (sizeof(hotwand_setup_nvm_t) / sizeof(uint16_t));
         ++halfword) {
                                  ...
                                  uint16_t trace_number)
{
    ...
    write_address = erase_expected
                        ? test_nvm_page_start()
                        : test_nvm_slot_address(next_slot);
    test_nvm_make_settings(sequence, &expected);
    ...
    api_read_ok = save_ok && nvm_read(&read_back) &&
                  test_nvm_settings_equal(&expected, &read_back);
    test_nvm_read_physical(write_address, &physical);
    physical_read_ok = save_ok &&
                       test_nvm_settings_equal(&expected, &physical);

    UART_Write(save_ok ? "SAVE API PASS\r\n" : "SAVE API FAIL\r\n");
    if (erase_expected) {
        UART_Write(erase_confirmed
                       ? "ERASE HAPPENED AND WAS CONFIRMED\r\n"
    ...
    return save_ok && api_read_ok && physical_read_ok &&
           (!erase_expected || erase_confirmed);
}
