...
void btn_init(void)
{
     ...
     * use the MCU pull-up and interrupt on both press and release edges.
     */
    ...
    now = systick_get_ms();
    btn_down =
        (HAL_GPIO_ReadPin(BTN_GPIOx, BTN_PINn) == GPIO_PIN_RESET);
     ...
     * not clear the shared NVIC pending bit, which could discard a tip edge.
     */
...
bool btn_is_down(void)
{
    ...
    return HAL_GPIO_ReadPin(BTN_GPIOx,
                            BTN_PINn) == GPIO_PIN_RESET;
}
...
void btn_task(void)
{
    ...
    if (btn_down && btn_release_pending) {
        if (pin_is_down) {
             ...
             * it as bounce.
             */
            if ((uint32_t)(now - btn_release_since_ms) >=
                BTN_DEBOUNCE_MS) {
                btn_accept_down(now);
            } else {
                btn_release_pending = false;
            }
        } else if ((uint32_t)(now - btn_release_since_ms) >=
                   BTN_DEBOUNCE_MS) {
            btn_down = false;
            btn_release_pending = false;
            btn_long_press_emitted = false;
