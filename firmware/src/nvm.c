...
#define NVM_RECORD_MAGIC       0xA5U
#define NVM_ERASED_HALFWORD    0xFFFFU
...
static bool nvm_settings_fields_are_valid(
    const hotwand_setup_nvm_t *settings);
...
static void nvm_prepare_record(const hotwand_setup_nvm_t *settings,
                               hotwand_setup_nvm_t *record);
static bool nvm_records_are_equal(const hotwand_setup_nvm_t *left,
                                  const hotwand_setup_nvm_t *right);
static void nvm_scan_page(void);
static bool nvm_erase_page(void);
static bool nvm_program_record(uint16_t slot,
                               const hotwand_setup_nvm_t *record);
...
bool nvm_save(const hotwand_setup_nvm_t *settings)
{
    ...
    if (!nvm_initialized ||
        !nvm_settings_fields_are_valid(settings)) {
        return false;
    }
    ...
    if (nvm_has_saved_settings &&
        nvm_records_are_equal(&record, &nvm_saved_settings)) {
        return true;
    }
...
static void nvm_read_record(uint16_t slot, hotwand_setup_nvm_t *record)
{
    const volatile uint8_t *source =
        (const volatile uint8_t *)nvm_slot_address(slot);
    uint8_t *destination = (uint8_t *)record;
    uint16_t index;

    for (index = 0U; index < NVM_SLOT_SIZE_BYTES; ++index) {
...
static bool nvm_slot_is_erased(uint16_t slot)
{
    ...
    for (offset = 0U; offset < NVM_SLOT_SIZE_BYTES; offset += 2U) {
...
static bool nvm_page_is_erased(void)
{
    ...
    for (address = nvm_page_start(); address < nvm_page_end();
         address += 2U) {
...
static uint16_t nvm_checksum(const hotwand_setup_nvm_t *settings)
{
    return fletcher16((const uint8_t *)settings,
                      offsetof(hotwand_setup_nvm_t, checksum));
}

static bool nvm_record_is_valid(const hotwand_setup_nvm_t *record)
{
    return (record->magic == NVM_RECORD_MAGIC) &&
           (record->rsvd_1 == 0U) &&
           (record->rsvd_2 == 0U) &&
           nvm_settings_fields_are_valid(record) &&
                               ...
                               const hotwand_setup_nvm_t *record)
{
     ...
     */
    for (offset = 2U; offset < NVM_SLOT_SIZE_BYTES; offset += 2U) {
        halfword = (uint16_t)bytes[offset] |
                   ((uint16_t)bytes[offset + 1U] << 8U);
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,
                                   (uint32_t)(address + offset),
                                   halfword);
        if ((status != HAL_OK) ||
            (nvm_read_halfword(address + offset) != halfword)) {
            return false;
        }
    }

    halfword = (uint16_t)bytes[0] | ((uint16_t)bytes[1] << 8U);
    status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,
                               (uint32_t)address,
                               halfword);
    ...
    return nvm_record_is_valid(&verify) &&
           nvm_records_are_equal(&verify, record);
}
