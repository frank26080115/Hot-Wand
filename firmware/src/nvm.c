...
static bool nvm_page_layout_is_valid(void)
{
    ...
    return (end > start) && ((end - start) == NVM_EXPECTED_PAGE_SIZE) && ((start % NVM_EXPECTED_PAGE_SIZE) == 0U) &&
           ((start % 2U) == 0U) && ((NVM_SLOT_SIZE_BYTES % 2U) == 0U);
}
...
static bool nvm_page_is_erased(void)
{
    ...
    for (address = nvm_page_start(); address < nvm_page_end(); address += 2U)
    {
...
static bool nvm_record_is_valid(const hotwand_setup_nvm_t* record)
{
    return (record->magic == NVM_RECORD_MAGIC) && (record->rsvd_1 == 0U) && (record->rsvd_2 == 0U) &&
...
static bool nvm_erase_page(void)
{
    ...
    if ((status != HAL_OK) || (page_error != 0xFFFFFFFFUL) || !nvm_page_is_erased())
    {
        nvm_scan_page();
        return false;
    }
...
static bool nvm_program_record(uint16_t slot, const hotwand_setup_nvm_t* record)
{
    ...
    {
        halfword = (uint16_t)bytes[offset] | ((uint16_t)bytes[offset + 1U] << 8U);
