...
static uint8_t setup_menu_draw_text(u8g2_t *graphics,
                                    const char *text,
                                    char terminator,
                                    uint8_t baseline);
static void setup_menu_draw_line(u8g2_t *graphics,
                                 const char *text,
                                 size_t length,
                                 uint8_t baseline);
static void setup_menu_render(u8g2_t *graphics,
...
void setup_menu(void)
{
    ...
    for (;;) {
        ...
        if (!input_enabled) {
            ...
            } else if ((uint32_t)(now - initial_release_ms) >=
                       BTN_DEBOUNCE_MS) {
                (void)btn_has_short_press(true);
                (void)btn_has_long_press(true);
                input_enabled = true;
        ...
        if (short_press_pending) {
            ...
            } else if ((uint32_t)(now - pending_release_ms) >=
                       BTN_DEBOUNCE_MS) {
                short_press_pending = false;
                pending_release_timing = false;
                selected_item = (uint8_t)((selected_item + 1U) %
                                          SETUP_MENU_ITEM_COUNT);
                last_activity_ms = now;
                setup_menu_render(graphics, &settings, selected_item);
            }
                              ...
                              uint8_t item)
{
    ...
    if ((graphics == NULL) || (settings == NULL) ||
        (item >= SETUP_MENU_ITEM_COUNT)) {
        return;
    }
    ...
    baseline = (uint8_t)(SETUP_MENU_FIRST_BASELINE +
                         (2U * SETUP_MENU_LINE_HEIGHT));
    baseline = setup_menu_draw_text(graphics,
                                    menu_item->title,
                                    '\0',
                                    baseline);
    ...
    if (item <= SETUP_MENU_LAST_VALUE_ITEM) {
        ...
        baseline = (uint8_t)(baseline + SETUP_MENU_LINE_HEIGHT);
        option = setup_menu_find_option(
            menu_item->items,
            setup_menu_get_value(settings, item));
